...
store <- read.csv("/cloud/project/DEPARTMENTAL STORE PROJECT/MY DEPARTMENTAL STORE.csv")
View(store)
...
summarise(store3, AVERAGE=mean(PROFIT, na.rm=TRUE))

##5.2.Find the SUMMATION
SUM_NET_PROFIT <- summarise(store3, SUM=sum(NET_PROFIT, na.rm=TRUE))
View(SUM_NET_PROFIT)

###5.3.Find the MINIMUM

summarise(store3, MINIMUM=min(PROFIT, na.rm=TRUE))

###5.4. Find the median 
summarise(store3, MEDIAN=median(PROFIT, na.rm=TRUE))

###5.5. Find the variance 
summarise(store3, VARIANCE=var(PROFIT, na.rm=TRUE))

###5.6. Find the standard deviation 
summarise(store3, STANDARD_DEVIATION=sd(PROFIT, na.rm=TRUE))

###SELF-ANALYSIS 3
##5.7. Find the MAXIMUM
### This one I leave for you to think and write. 
###ALL THE BEST!
summarise(store3, MAXIMUM=max(PROFIT, na.rm=TRUE))
